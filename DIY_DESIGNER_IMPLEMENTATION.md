...
- Local-only native APS SketchUp handoff through a loopback-only, random one-time `apsImport` descriptor. The designer fetches the same editable schema-v2 JSON, validates it, passes it through the ordinary non-destructive import decision, and removes the descriptor from the URL. This has not been deployed to the public site; manual JSON remains available and the public `apsBridge` parameter stays isolated for the optional legacy browser extension.
...
- Other hardware now includes a fixed-black threaded-stem caster (M6/M8/M10/M12, brake/no-brake), the fixed-gold referenced 70×70mm adjustable foot (60-68mm, Ø28mm post, M8 adjustment point), and a color-selectable aluminum profile end cap bound to a specific profile end. The end cap automatically adds missing end tapping and only removes tapping that it added itself. Confirmed prices are caster ¥18 base and adjustable foot ¥10; existing caster option surcharges remain brake +¥4, M10 +¥2, and M12 +¥4. Confirmed end-cap prices are 2020 ¥3 retail / ¥2.7 bulk and 3030 ¥4 retail / ¥3.5 bulk, with every color at the same price and the existing 20-piece same-line bulk threshold. Other end-cap sizes keep the clearly labelled provisional ¥6 natural / ¥8 colored fallback until confirmed.
- Complete front profile frames automatically enable an auto-fit cabinet-door tool. Adjacent vertical boundaries split an N-column frame into N independently bound door leaves. Full-overlay leaves keep 3mm clearance around the outside frame and a 3mm total seam between neighboring leaves; inset/large-bend leaves fit inside each column's clear opening with a 3mm-per-side perimeter gap. Doors can switch among aluminum, marine board, and aluminum pegboard; full overlay, half overlay, and inset/large-bend hinges; and left/right opening. Aluminum-frame and aluminum-pegboard doors both use an 18mm finished door depth in the 3D scene and manufacturing data; the pegboard door still uses the established 2mm pegboard material rate for pricing. The normal closed view keeps concealed hinges hidden and shows them only in translucent-profile inspection mode; handles remain visible, and a selected door always receives a clear cyan outline. JSON/XLSX/cart/factory data retain the door material, overlay, opening direction, hinge positions, frame relationship, and price metadata. The final production-sheet/PDF projection groups physically identical panels and door leaves by complete manufacturing specification and sums their quantities; generated location remarks are retained but do not split identical rows, while the editable designer scene, cart, JSON, and XLSX remain per-piece.
...
- Browser-tested the default 1200×600×2200 wardrobe before the 2026-08-24 clearance update: the empty designer keeps the door disabled, the generated front frame enables it, and material plus left/right-opening changes reach the 3D model, cart, and factory sheet. With the current 3mm-per-side rule, a single-column full/half/inset leaf is expected to produce 1194×2194 / 1174×2174 / 1154×2154mm; the earlier 2mm-per-side dimensions were 1196×2196 / 1176×2176 / 1156×2156mm.
