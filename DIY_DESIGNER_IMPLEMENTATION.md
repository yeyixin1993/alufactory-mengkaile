...
The library's purple installation markers hide occupied positions. The separate “Edit accessories” mode retains its green installed-joint markers as explicit delete controls; this removal gesture is unchanged.

Nearby purple markers now share a numbered screen-space entry with a large face picker, hover/focus preview, and explicit install button. All physical XY/XZ/YZ corner planes remain available; a successful install removes just that face, with stale-candidate validation before commit.

The designer header has Back to home. It writes the complete editable scene to an account-scoped browser draft before navigation, restores it on the next normal entry, gives explicit furniture-template handoffs priority, and clears stale drafts when leaving an empty scene. Cache failure displays an error and prevents navigation; design JSON remains the portable backup. Run `node scripts/test-designer-draft.mjs` for persistence boundary tests.

Door sizing clarification (2026-09-06): full and half overlay share the same height; only inset reduces height. Each paired leaf is independently editable. Width/height floor to integer millimetres throughout scene/import/export. A fixed opening centre preserves the 3mm pair seam across mixed overlays, with any fractional cutting remainder allocated to the outer edge. This supersedes previous exact-fractional-dimension assumptions below.

Browser regression (2026-09-06): switching only the left leaf of a 1200×600×2200 wardrobe from full to half changes it from 595×2194 to 585×2194mm; its right sibling remains full at 595×2194mm. A three-entry No.2 face cluster drops to two after one install and the item count increases by exactly one. Back to home → return restores all 35 test items and the unchanged ¥682.4 estimate. Production build and the three door/draft/import-tapping regression scripts pass; repository-wide TypeScript checking retains four known baseline errors (backend alias, App timeout narrowing, XLSX fastener-direction widening, accessory-kind narrowing).

Run `node scripts/test-diy-door-placement.mjs` to check paired-door geometry, JSON metadata recovery, production XLSX metadata, and occupied No.1/No.2 placement markers.

- New `#/diy-designer` route linked from the catalog and desktop navigation.
- Three.js scene with orbit camera, lighting, shadows, grid, selection, six signed world-axis move handles, and direct in-scene 90-degree rotation controls. The scene intentionally has no distance fog: profiles, panels, doors, and accessories retain their normal color and contrast when the customer zooms out. Left-drag always orbits; after selection, left-dragging a ±X/±Y/±Z arrow performs constrained movement, while right-click without dragging keeps the rotate/delete menu. Every accessory—including generated attached brackets—uses the same translation gizmo; moving one releases its old attachment. The colored translation gizmo stays at the selected item's geometric centre (the midpoint for profiles), while black end-resize handles remain profile-only and stay at the two ends.
...
- Persistent “Edit accessories” mode for real aligned 2020/3030-system perpendicular contacts: first choose No.1 outside corner bracket, No.5 groove connector, or drill-and-tap, then click purple points to add or orange points to replace. A successfully occupied position is removed immediately from the purple add-candidate set; explicit item deletion and delete-all remain available. Add-all and delete-all operate on the current type across eligible joints. Profiles turn translucent during editing; generated relationships remain geometry-positioned, locked, and idempotent.
- Flat No.7 T/L plates are accepted only when both target profiles expose one coplanar exterior mounting face. Both resolve from the true 90-degree contact, fill the matched profile module, and require every corner of each real arm to remain supported: L starts both arms at the contact; T moves its visual origin half a module from the perpendicular butt face to the continuous profile centreline, centres its complete crossbar there, and starts its complete stem there. Each arm keeps two fasteners over its assigned profile. Third-profile blocking tests the real L/T footprints rather than their empty bounding rectangles. No.1 is a full-width die-cast corner seat (2020: 20/28mm legs and 20mm width; 3030: 30/30/30mm) with two face-normal socket fasteners. No.2 shares No.1's persistent purple-hotspot, repeated-installation interaction but keeps its own 50×50mm leg-clearance and collision checks. Both bracket types install only at the true inside corner of two perpendicular contacting profiles, where both seating faces remain flush and the body stays outside both extrusions. Their neighbouring half-module collision probes are compacted to one stable customer-facing purple hotspot per real profile-pair/orientation; already-installed attachment keys remain the preferred representative so repeat clicks replace rather than duplicate.
...
- Shared profile/production previews mirror multi-port end-tapping markers on D using the drilling `physicalGrooveToDisplay` mapping. Saved arrays and toggle callbacks retain physical port indices, so a B-slot drill and its corresponding tapped end port stay matched from either view. Run `node scripts/test-profile-tapping-view.mjs` for rendered-marker and click-mapping regressions.
- JSON and MayCAD profile imports share a post-import both-end-tapping choice. JSON offers “keep existing tapping” rather than clearing source machining. Append/replace resolves first; tapping targets only the actual imported profile IDs and remains undoable.
...
- Undo/redo, duplicate/delete, local save/load, and JSON export/import support. JSON, production-Excel, and MayCAD imports into a non-empty canvas use an explicit add/replace/cancel dialog; appended assemblies receive fresh item/hole IDs with their internal profile, screw, hole, joint, and accessory relationships remapped as one batch. Sending a design to a non-empty cart uses the same add/replace/cancel decision instead of silently appending or clearing customer goods.
- The maintained external-AI prompt supports exact `assembly` JSON and coordinate-free `bom-staging` JSON. Staging expands every BOM quantity into independent `quantity=1` profiles, keeps their exact machining/end-tapping data, and lays them out as non-interfering 150mm-spaced rows. These records use ordinary supported scene items, so current pricing, color editing, cart conversion, and production-PDF grouping work without claiming that the staged row positions reproduce the source assembly. The prompt also documents the verified SketchUp 20-series metadata and axis conversion.
...
- The special 2047 section renders one sealed 20mm A face, with the opposite 20mm edge and both 47mm faces grooved, matching its drilling-side map and retaining three independent tapping ports per end.
...
- Complete front profile frames automatically enable an auto-fit cabinet-door tool. Each detected column can generate either one single-opening leaf or a left/right pair. Pair doors keep a 3mm centre seam and hinge on the two outside edges. Full-overlay leaves keep 3mm clearance around the outside frame and a 3mm total seam between neighboring leaves; restored half-overlay leaves cover half of the profile width and retain the same 3mm clearance; inset/large-bend leaves fit inside each column's clear opening with a 3mm-per-side perimeter gap. Doors can switch among aluminum, marine board, and aluminum pegboard; full overlay, half overlay, and inset/large-bend hinges; and single-leaf left/right opening. Aluminum-frame and aluminum-pegboard doors both use an 18mm finished door depth in the 3D scene and manufacturing data; the pegboard door still uses the established 2mm pegboard material rate for pricing. The normal closed view keeps concealed hinges hidden and shows them only in translucent-profile inspection mode; handles remain visible, and a selected door always receives a clear cyan outline. JSON/XLSX/cart/factory data retain the door material, overlay, single/pair layout, pair side, opening direction, hinge positions, frame relationship, and price metadata. The final production-sheet/PDF projection groups physically identical panels and door leaves by complete manufacturing specification and sums their quantities; generated location remarks are retained but do not split identical rows, while the editable designer scene, cart, JSON, and XLSX remain per-piece.
...
- Browser-tested the default 1200×600×2200 wardrobe: the empty designer keeps the door disabled and the generated front frame enables it. With the current 3mm-per-side and equal full/half-height rules, a single-column full/half/inset leaf produces 1194×2194 / 1174×2194 / 1154×2154mm. Integer cutting and all mixed pair-overlay combinations are covered by the door geometry regression script.
