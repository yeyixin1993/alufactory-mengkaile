...
export const buildProductionXlsx = (production: ProductionWorkbookData) => {
  ...
  const partRows = production.parts.map((part) => [
    ...
    part.thicknessMm ?? '', part.finish ?? '', part.shelfSupportType ?? '', part.color, part.colorId ?? '', part.quantity, part.pegHolePattern ?? '', part.positionMm.join(', '), part.rotationDeg.join(', '),
