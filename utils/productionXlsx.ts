...
export const buildProductionXlsx = (production: ProductionWorkbookData) => {
  ...
  const partRows = production.parts.map((part) => [
    ...
    part.thicknessMm ?? '', part.finish ?? '', part.shelfSupportType ?? '', part.fixedReferenceId ?? '', part.shaftDiameterMm ?? '', part.color, part.colorId ?? '', part.quantity, part.pegHolePattern ?? '', part.positionMm.join(', '), part.rotationDeg.join(', '),
  ...
  const sheets: SheetDefinition[] = [
    ...
    },
    {
      ...
      headers: ['序号', '零件ID', '类型', '型号', '适配型材规格', '长度mm', '宽度mm', '高度mm', '厚度mm', '截面/表面档位', '层板托类型', '固定库ID', '轴径mm', '颜色', '颜色ID', '数量', '洞洞板孔型', '位置XYZ(mm)', '旋转XYZ(°)', '左端攻丝孔数', '右端攻丝孔数', '螺丝头型', '关联型材ID', '关联孔ID', '自动生成', '位置锁定', '连接型材ID', '连接关系ID', '配件螺纹规格', '带刹车', '端盖端面', '端盖自动攻丝', '柜门材料', '柜门覆盖方式', '柜门开向', '备注'],
      rows: partRows,
      widths: [8, 25, 14, 14, 16, 12, 12, 12, 12, 18, 16, 26, 11, 15, 14, 9, 14, 20, 20, 15, 15, 14, 25, 25, 12, 12, 38, 38, 16, 12, 12, 16, 14, 16, 12, 38],
    },
    {
      name: '打孔明细',
