...
export const buildDisplayRack3Template = (
  ...
  layout.upperTierHeightsMm.forEach((tierHeight, index) => {
    ...
    items.push({
      ...
      quantity: 1,
      length: width,
        width: DISPLAY_RACK_COMPONENT_CATALOG.SHAFT_8.diameterMm,
        height: 8,
      thickness: width,
      shelfSupportType: 'linear_shaft',
      shaftDiameterMm: DISPLAY_RACK_COMPONENT_CATALOG.SHAFT_8.diameterMm,
      accessoryPrice: 0,
  ...
  const slideLength = slideCatalog.availableLengthsMm.find((length) => length <= depth - 50)
    || slideCatalog.availableLengthsMm[slideCatalog.availableLengthsMm.length - 1];
  layout.drawerCenterHeightsMm.forEach((centerHeight, index) => {
    const level = index + 1;
    ...
    items.push({
      ...
      autoGenerated: true,
      lockedPosition: true,
      remark: `3.0展架第${level}层三节滚珠抽屉滑轨；一套含左轨和右轨，单侧安装间隙固定${slideCatalog.sideClearanceMm}mm，标准长度${slideLength}mm，价格待录入`,
    });
  });
