...
const profileAccessorySeries = (item: DIYSceneItem): DIYAccessoryProfileSize | null => {
  ...
  const [width, height] = profileSize(item.variantId);
  const moduleSize = Math.min(width, height);
  const series = `${moduleSize}${moduleSize}` as DIYAccessoryProfileSize;
  return ACCESSORY_PROFILE_SIZES.includes(series) ? series : null;
};
