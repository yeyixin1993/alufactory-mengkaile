...
from app.profile_inventory import PROFILE_COLORS, _cell_value, _column_name, _xlsx_cell
...
ACCESSORY_COLORS = PROFILE_COLORS
ACCESSORY_INVENTORY_HEADERS = ('配件ID', '编号', '配件名称', '适配型号', '颜色ID', '颜色名称', '库存数量')
...
def serialize_accessory_inventory(row):
    ...
    data.update({
        ...
        'color_name': ACCESSORY_COLORS.get(row.color_id, row.color_id),
    })
    return data
...
def seed_accessory_inventory():
    existing = {
        ...
        for row in AccessoryInventory.query.all()
    }
    additions = []
    for accessory_id, definition in ACCESSORY_CATALOG.items():
        for profile_size in definition['sizes']:
            for color_id in ACCESSORY_COLORS:
                key = (accessory_id, profile_size, color_id)
...
def public_accessory_stock():
    return [
        ...
        for row in AccessoryInventory.query.order_by(
...
def build_accessory_inventory_xlsx(rows):
    ...
    data_rows.extend((
        ...
        row.profile_size,
        row.color_id,
        ACCESSORY_COLORS.get(row.color_id, row.color_id),
    ...
    for row_index, values in enumerate(data_rows, start=1):
        cells = ''.join(
            _xlsx_cell(
                ...
                1 if row_index == 1 else (2 if column_index == 7 else 0),
            )
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:G{len(data_rows)}"/>
  ...
  <cols><col min="1" max="1" width="28" customWidth="1"/><col min="2" max="2" width="10" customWidth="1"/><col min="3" max="3" width="34" customWidth="1"/><col min="4" max="7" width="16" customWidth="1"/></cols>
  <sheetData>{''.join(sheet_rows)}</sheetData><autoFilter ref="A1:G{len(data_rows)}"/>
</worksheet>'''
...
def parse_accessory_inventory_xlsx(file_stream):
    ...
    with ZipFile(file_stream) as archive:
        ...
        for row in root.findall('.//m:sheetData/m:row', namespace):
            ...
            parsed_rows.append([values.get(column, '') for column in ('A', 'B', 'C', 'D', 'E', 'F', 'G')])
    if not parsed_rows:
        raise ValueError('Excel中没有库存数据')
    if [str(value).strip() for value in parsed_rows[0]][:7] != list(ACCESSORY_INVENTORY_HEADERS):
    ...
    for row_number, values in enumerate(parsed_rows[1:], start=2):
        ...
        if color_id not in ACCESSORY_COLORS:
            raise ValueError(f'第{row_number}行颜色ID不支持：{color_id}')
        if quantity < 0:
            raise ValueError(f'第{row_number}行库存数量不能为负数')
        records.append((accessory_id, profile_size, color_id, quantity))
    return records


def apply_accessory_inventory_records(records):
    updated = 0
    for accessory_id, profile_size, color_id, quantity in records:
