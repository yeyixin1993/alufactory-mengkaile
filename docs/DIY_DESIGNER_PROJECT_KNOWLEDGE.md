...
- Intended product behavior is for the internal 3D screw geometry to use the same catalog length as cart/PDF output. The current scene still has legacy dynamically fitted 27/28mm rendering metadata in some paths; this is known implementation debt and is deliberately deferred to a later 3D-geometry fix. Until then, those fitted values remain internal only and must never replace the purchasing length shown to the customer or factory.
