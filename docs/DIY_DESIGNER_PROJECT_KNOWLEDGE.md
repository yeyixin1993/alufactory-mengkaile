...
- **2026-08-10:** Added parameterized furniture templates that enter the normal editable designer rather than maintaining a separate renderer. The IKEA calligraphy-basket cabinet uses 300×420×100mm basket bays, 130mm vertical pitch, fixed 460mm outside depth, all-2020 frame members, continuous full-width front/rear top and bottom rails, shared 420mm depth rails on each divider, exactly two shelf-support placeholders per basket, and one continuous 12mm marine-board top. Overall generated length is `columns×300 + (columns+1)×20`; overall height is `layers×130 + 40`. Length/height bound mode chooses the largest whole grid that does not exceed the customer's input. The one-piece marine-board top caps overall length at 2440mm (seven columns). Shelf supports remain unpriced placeholders until an authoritative catalog SKU and price are supplied, and the UI must disclose this before ordering.
