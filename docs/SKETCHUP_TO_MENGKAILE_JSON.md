...
- 1.1 起同时输出统一的 `provenance` 来源凭证（导出器、版本、模型名、唯一文档 ID 和导出时间）。设计器、购物车和订单会原样保留这段凭证，后台可按来源渠道统计。
- 本地 JSON 的来源凭证属于 `self_declared`，用于追溯和统计，不代表官网防伪认证；未来只有经过登录与服务端签名的一键传输才能标记为 `server_verified`。
- 不输出 SU 中的价格。设计器依据当前型材、颜色、长度、加工和会员规则重新报价。
- 当前 1.1 导出器只导出已识别型材及明确写入型材属性的孔/攻丝。板材、门和连接配件以后必须通过各自明确的 SKU/安装关系属性扩展，不能从形状猜测。
