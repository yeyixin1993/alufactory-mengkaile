...
assert.equal((guest.match(/class="mkl-page /g) || []).length, 16);
...
for (const membershipLevel of ['vip_plus', 'VIP+']) {
  ...
  assert.equal((vip.match(/class="mkl-page /g) || []).length, 17);
  const appendix = vip.indexOf('<section class="mkl-page mkl-page-17 " id="catalog-vip-plus"');
  assert(appendix > 0);
...
for (const language of ['en', 'jp'] as const) {
  ...
  assert.equal((regular.match(/class="mkl-page /g) || []).length, 16);
  const member = renderToStaticMarkup(<CatalogPages language={language} user={{ ...user, membershipLevel: 'vip_plus' }} />);
  assert.equal((member.match(/class="mkl-page /g) || []).length, 17);
  const appendix = member.indexOf('<section class="mkl-page mkl-page-17 ');
